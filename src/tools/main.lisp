...
(defun sync-scryfall-data ()
  (let ((mito:*connection* (dbi:connect :mysql :database-name (uiop:getenv "MALAGA_DB") :username (uiop:getenv "MALAGA_MYSQL_USERNAME") :password (uiop:getenv "MALAGA_MYSQL_PASSWORD") :port 3306))
        (output nil))
      (setf output (unwind-protect (progn (handler-case (malaga/tools/utils:with-file-lock (pathname (uiop:getenv "MALAGA_LOCK"))
        (malaga/tools/scryfall:sync-data))
        (malaga/tools/utils:lock-exists-error (err) (format t "Can't update, ~A~%" (malaga/tools/utils:message err)))))))
...
(defun sync-player-data ()
  (let ((mito:*connection* (dbi:connect :mysql :database-name (uiop:getenv "MALAGA_DB") :username (uiop:getenv "MALAGA_MYSQL_USERNAME") :password (uiop:getenv "MALAGA_MYSQL_PASSWORD") :port 3306))
      ...
      (setf output (unwind-protect (progn
        (handler-case (malaga/tools/utils:with-file-lock (pathname (uiop:getenv "MALAGA_LOCK"))
